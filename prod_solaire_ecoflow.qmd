...
quot[, an := year(date)][, mois := month(date)][, semaine := isoweek(date)][, trim := as.factor(quarter(date))][, jour := as.factor(wday(date, week_start = 1))][, mois_an := format(date, "%Y-%m")][, semaine_an := date - (as.integer(strftime(date, "%u")) - 1)]
...
mean_prod_hebd2 = mean(prod_hebd2[1:(.N-1), prod_kWh])
...
ggplot() +
  ...
  geom_text(data = prod_hebd2, aes(x = semaine_an, y = prod_kWh, label = round(prod_kWh)), vjust = 1.3, size = 4) + # place le texte au-dessus des barres
...
La production hebdomadaire moyenne (sans la semaine en cours) est de `r round(mean_prod_hebd2,3)` kWh.
...
### Classements des 10 jours les plus et les moins productifs\*

\*Sans les jours à production nulle
...
quot2 <- subset(quot, date > "2025-05-16" & production >= 0.001, select=c("date", "production"))
...
quot2[, an := year(date)]
quot25 <- subset(quot2, an == 2025)
quot26 <- subset(quot2, an == 2026)

tab_plus25 <- quot25[1:5, ]
tab_moins25 <- quot25[(.N - 4):.N, ]

tab_plus26 <- quot26[1:5, ]
tab_moins26 <- quot26[(.N - 4):.N, ]
...
| 2025 |   | 2026 |   |
|------------------|------------------|------------------|------------------|
| Date | Production (en Wh) | Date | Production (en Wh) |
| `r tab_plus25[1, date]` | `r tab_plus25[1, production]` | `r tab_plus26[1, date]` | `r tab_plus26[1, production]` |
| `r tab_plus25[2, date]` | `r tab_plus25[2, production]` | `r tab_plus26[2, date]` | `r tab_plus26[2, production]` |
| `r tab_plus25[3, date]` | `r tab_plus25[3, production]` | `r tab_plus26[3, date]` | `r tab_plus26[3, production]` |
| `r tab_plus25[4, date]` | `r tab_plus25[4, production]` | `r tab_plus26[4, date]` | `r tab_plus26[4, production]` |
| `r tab_plus25[5, date]` | `r tab_plus25[5, production]` | `r tab_plus26[5, date]` | `r tab_plus26[5, production]` |

**Les 5 moins bon jours par année**

| 2025 |   | 2026 |   |
|------------------|------------------|------------------|------------------|
| Date | Production (en Wh) | Date | Production (en Wh) |
| `r tab_moins25[1, date]` | `r tab_moins25[1, production]` | `r tab_moins26[1, date]` | `r tab_moins26[1, production]` |
| `r tab_moins25[2, date]` | `r tab_moins25[2, production]` | `r tab_moins26[2, date]` | `r tab_moins26[2, production]` |
| `r tab_moins25[3, date]` | `r tab_moins25[3, production]` | `r tab_moins26[3, date]` | `r tab_moins26[3, production]` |
| `r tab_moins25[4, date]` | `r tab_moins25[4, production]` | `r tab_moins26[4, date]` | `r tab_moins26[4, production]` |
| `r tab_moins25[5, date]` | `r tab_moins25[5, production]` | `r tab_moins26[5, date]` | `r tab_moins26[5, production]` |
...
nb_jour_inf <- quot[production >= 0.001 & production < 1000, .N]
nb_jour_sup <- quot[production > 5000, .N]
prod_an1 <- quot2[date >= "2025-05-15" & date <= "2026-05-14", .(prod = (sum(production)/1000))]
prod_an2 <- quot2[date >= "2026-05-15", .(prod = (sum(production)/1000))]

```

Nombre de jours avec production supérieure à 5 kWh: `r nb_jour_sup`

Nombre de jours avec production inférieure à 1 kWh: `r nb_jour_inf`

Production entre le 15 mai 2025 et le 14 mai 2026: `r prod_an1$prod` kWh.

Production depuis le 15 mai 2026: `r prod_an2$prod` kWh.
...
# Quotidien
conso_quot2 <- subset(conso_quot, date >= j90)

mean_conso_quot = mean(conso_quot2[, consommation])
mean_conso_kwh = mean_conso_quot/1000
sd_conso_quot = sd(conso_quot2[, consommation])

ggplot(conso_quot2, aes(x = date)) +
  ...
  labs(x = "Date", y = "Consommation (en Wh)", title = "Consommation quotidienne (90 derniers jours)")
...
# Calcul d'une moyenne
mean_conso_hebd2 = mean(conso_hebd2[1:(.N-1), conso_kWh])
...
ggplot() +
  ...
  geom_text(data = conso_hebd2, aes(x = semaine_an, y = conso_kWh, label = round(conso_kWh)), vjust = 1.3, size = 4) +
...
La consommation hebdomadaire moyenne (sans la semaine en cours) est de `r round(mean_conso_hebd2,3)` kWh.
...
ggplot() +
  ...
  geom_text(data = conso_mens,
            aes(x = mois_an, y = conso_kWh, label = round(conso_kWh)),
            vjust = 1.3,  # place le texte au-dessus des barres
            size = 4) +
...
Prise en compte de la batterie: on s'intéresse au moment où l'energie est envoyée à la maison
...
```{r}
#| label: autoconso_graph2
#| echo: false
#| warning: false

# Graphique
ggplot() +
  geom_area(data = df_conso_aff, aes(x = Heure, y = Consommation, fill = "Consommation"), alpha = 0.6) +
  geom_area(data = df_maison_aff, aes(x = Heure, y = Production, fill = "Production"), alpha = 0.45) +
...
**Taux d'autoconsommation et de couverture des 10 derniers jours**

| Date | Autoconsommation (en %) | Couverture (en %) |
|------------------------|------------------------|------------------------|
| `r dt_auto[.N, date]` | `r dt_auto[.N, round(tx_auto, 1)]` | `r dt_auto[.N, round(tx_couverture, 1)]` |
| `r dt_auto[.N-1, date]` | `r dt_auto[.N-1, round(tx_auto, 1)]` | `r dt_auto[.N-1, round(tx_couverture, 1)]` |
| `r dt_auto[.N-2, date]` | `r dt_auto[.N-2, round(tx_auto, 1)]` | `r dt_auto[.N-2, round(tx_couverture, 1)]` |
| `r dt_auto[.N-3, date]` | `r dt_auto[.N-3, round(tx_auto, 1)]` | `r dt_auto[.N-3, round(tx_couverture, 1)]` |
| `r dt_auto[.N-4, date]` | `r dt_auto[.N-4, round(tx_auto, 1)]` | `r dt_auto[.N-4, round(tx_couverture, 1)]` |
| `r dt_auto[.N-5, date]` | `r dt_auto[.N-5, round(tx_auto, 1)]` | `r dt_auto[.N-5, round(tx_couverture, 1)]` |
| `r dt_auto[.N-6, date]` | `r dt_auto[.N-6, round(tx_auto, 1)]` | `r dt_auto[.N-6, round(tx_couverture, 1)]` |
| `r dt_auto[.N-7, date]` | `r dt_auto[.N-7, round(tx_auto, 1)]` | `r dt_auto[.N-7, round(tx_couverture, 1)]` |
| `r dt_auto[.N-8, date]` | `r dt_auto[.N-8, round(tx_auto, 1)]` | `r dt_auto[.N-8, round(tx_couverture, 1)]` |
| `r dt_auto[.N-9, date]` | `r dt_auto[.N-9, round(tx_auto, 1)]` | `r dt_auto[.N-9, round(tx_couverture, 1)]` |

### Alimentation depuis ou injection vers le réseau

L'injection d'énergie vers le réseau est calculée sur la base des résultats horaires (Consommation - Energie envoyée vers la maison) et diffère donc des relevés effectués par Oiken.
...
df_reseau_quot <- subset(df_reseau_quot, date >= date_ref_min)
...
ggplot(df_reseau_quot, aes(x = date, y = quant_quot, fill = type)) +
  ...
  labs(title ="Utilisation du réseau", x = "Date", y = "Wh", fill = "",subtitle = paste("30 derniers jours")) +
  theme_gray() +
  theme(axis.text.x = element_text(angle=90, vjust = 0.5, hjust = 1), plot.title = element_text(hjust=0.5)) +
  theme(legend.position = "bottom")


# Préparation pour le tableau final
df_reseau_alim <- subset(df_reseau_quot, type == "alimentation")
df_reseau_inj <- subset(df_reseau_quot, type == "injection")

df_reseau_alim[, alim := quant_quot][, quant_quot := NULL]
df_reseau_inj[, inj := -quant_quot][, quant_quot := NULL]

# Date min
date_ref <- min(c(dt_auto[.N, date], conso_quot[.N, date], quot[.N, date], df_reseau_alim[.N, date]))


```

**Résumé des 10 derniers jours (en Wh)**

| Date | Consommation | Production | Alimentation depuis le réseau | Injection vers le réseau |
|---------------|---------------|---------------|---------------|---------------|
| `r dt_auto[date == date_ref, date]` | `r conso_quot[date == date_ref, round(consommation,0)]` | `r quot[date == date_ref, production]` | `r df_reseau_alim[date == date_ref, round(alim,0)]` | `r df_reseau_inj[date == date_ref, round(inj,0)]` |
| `r dt_auto[date == date_ref-1, date]` | `r conso_quot[date == date_ref-1, round(consommation,0)]` | `r quot[date == date_ref-1, production]` | `r df_reseau_alim[date == date_ref-1, round(alim,0)]` | `r df_reseau_inj[date == date_ref-1, round(inj,0)]` |
| `r dt_auto[date == date_ref-2, date]` | `r conso_quot[date == date_ref-2, round(consommation,0)]` | `r quot[date == date_ref-2, production]` | `r df_reseau_alim[date == date_ref-2, round(alim,0)]` | `r df_reseau_inj[date == date_ref-2, round(inj,0)]` |
| `r dt_auto[date == date_ref-3, date]` | `r conso_quot[date == date_ref-3, round(consommation,0)]` | `r quot[date == date_ref-3, production]` | `r df_reseau_alim[date == date_ref-3, round(alim,0)]` | `r df_reseau_inj[date == date_ref-3, round(inj,0)]` |
| `r dt_auto[date == date_ref-4, date]` | `r conso_quot[date == date_ref-4, round(consommation,0)]` | `r quot[date == date_ref-4, production]` | `r df_reseau_alim[date == date_ref-4, round(alim,0)]` | `r df_reseau_inj[date == date_ref-4, round(inj,0)]` |
| `r dt_auto[date == date_ref-5, date]` | `r conso_quot[date == date_ref-5, round(consommation,0)]` | `r quot[date == date_ref-5, production]` | `r df_reseau_alim[date == date_ref-5, round(alim,0)]` | `r df_reseau_inj[date == date_ref-5, round(inj,0)]` |
| `r dt_auto[date == date_ref-6, date]` | `r conso_quot[date == date_ref-6, round(consommation,0)]` | `r quot[date == date_ref-6, production]` | `r df_reseau_alim[date == date_ref-6, round(alim,0)]` | `r df_reseau_inj[date == date_ref-6, round(inj,0)]` |
| `r dt_auto[date == date_ref-7, date]` | `r conso_quot[date == date_ref-7, round(consommation,0)]` | `r quot[date == date_ref-7, production]` | `r df_reseau_alim[date == date_ref-7, round(alim,0)]` | `r df_reseau_inj[date == date_ref-7, round(inj,0)]` |
| `r dt_auto[date == date_ref-8, date]` | `r conso_quot[date == date_ref-8, round(consommation,0)]` | `r quot[date == date_ref-8, production]` | `r df_reseau_alim[date == date_ref-8, round(alim,0)]` | `r df_reseau_inj[date == date_ref-8, round(inj,0)]` |
| `r dt_auto[date == date_ref-9, date]` | `r conso_quot[date == date_ref-9, round(consommation,0)]` | `r quot[date == date_ref-9, production]` | `r df_reseau_alim[date == date_ref-9, round(alim,0)]` | `r df_reseau_inj[date == date_ref-9, round(inj,0)]` |
